#'
#' Updated loop that should actually work 
#' shows regular CSA results as well
#' Updated Aug 2022 Julia Hatamyar
...
didMLloop <- function(dta = dta,
  ...
  for (g in 1:(nG)) { ## must subtract one here because there is no year at the end without TX?
    ...
    for (t in 1:tlist.length) {
      ...
      ## 
     # X <-as.matrix(cbind(disdat$X1,disdat$X2, disdat$X3))
      #X.SA <-as.matrix(cbind(disdat.SA$X1,disdat.SA$X2, disdat.SA$X3))
